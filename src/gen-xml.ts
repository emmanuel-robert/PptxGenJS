	...
	slide._slideObjects.forEach((slideItemObj: ISlideObject, idx: number) => {
		...
		switch (slideItemObj._type) {
			...
			case SLIDE_OBJECT_TYPES.image:
				...
				if (slideItemObj.options.sId != undefined) {
					strSlideXml += `<p:cNvPr id="${slideItemObj.options.sId}" name="Object ${idx + 1}" descr="${encodeXmlEntities(slideItemObj.options.altText || slideItemObj.image)}">`
				} else {
					strSlideXml += `<p:cNvPr id="${idx + 2}" name="Object ${idx + 1}" descr="${encodeXmlEntities(slideItemObj.options.altText || slideItemObj.image)}">`
				}
			...
			case SLIDE_OBJECT_TYPES.chart:
				...
				if (slideItemObj.options.sId != undefined) {
					strSlideXml += `   <p:cNvPr id="${slideItemObj.options.sId}" name="Chart ${idx + 1}" descr="${encodeXmlEntities(slideItemObj.options.altText || '')}"/>`
				} else {
					strSlideXml += `   <p:cNvPr id="${idx + 2}" name="Chart ${idx + 1}" descr="${encodeXmlEntities(slideItemObj.options.altText || '')}"/>`
				}
